...
#define LINUX_ENUMERATE_DEVICE_EXCPT(r, linuxAudio, method, message)                                            \
	...
	if (result != SND_OK)                                                                                       \
	{                                                                                                           \
		HEPH_RAISE_EXCEPTION(linuxAudio, ExternalException(method, message, "ALSA", snd_strerror(result))); \
...
#define LINUX_EXCPT(r, linuxAudio, method, message)                                                                       \
	...
	if (result < SND_OK)                                                                                                  \
	{                                                                                                                     \
		HEPH_RAISE_AND_THROW_EXCEPTION(linuxAudio, ExternalException(method, message, "ALSA", snd_strerror(result))); \
	}
	...
	namespace Native
	{
		...
		void LinuxAudio::SetMasterVolume(double volume)
		{
			if (isRenderInitialized && renderDeviceId != "")
			{
				...
				{
					HEPH_RAISE_AND_THROW_EXCEPTION(this, NotFoundException(HEPH_FUNC, "No mixer element found."));
				}

				if (!snd_mixer_selem_has_playback_volume(mixerElem))
				{
					HEPH_RAISE_AND_THROW_EXCEPTION(this, NotFoundException(HEPH_FUNC, "No mixer playback volume element found."));
				}
		...
		void LinuxAudio::GetNativeParams(NativeAudioParams& nativeParams) const
		{
			...
			{
				HEPH_RAISE_EXCEPTION(this, InvalidArgumentException(HEPH_FUNC, "nativeParams must be a AlsaParams instance."));
				return;
			}
