...
#define APPLE_ENUMERATE_DEVICE_EXCPT(r, appleAudio, method, message) result = r; if (result != kAudioHardwareNoError) { if (deviceIDs != nullptr) { free(deviceIDs); } HEPH_RAISE_EXCEPTION(appleAudio, ExternalException(method, message, "CoreAudio", "")); return NativeAudio::DEVICE_ENUMERATION_FAIL; }
#define APPLE_ENUMERATE_DEVICE_SOURCES_EXCPT(r, appleAudio, method, message) result = r; if (result != kAudioHardwareNoError) { free(deviceSources); HEPH_RAISE_EXCEPTION(appleAudio, ExternalException(method, message, "CoreAudio", "")); return NativeAudio::DEVICE_ENUMERATION_FAIL; }
#define APPLE_EXCPT(r, appleAudio, method, message) result = r; if (result != kAudioHardwareNoError) { HEPH_RAISE_AND_THROW_EXCEPTION(appleAudio, ExternalException(method, message, "CoreAudio", "")); }
...
namespace HephAudio
{
	namespace Native
	{
		...
		{
			HEPH_RAISE_AND_THROW_EXCEPTION(this, NotImplementedException(HEPH_FUNC, "Not implemented."));
		}
		...
		bool AppleAudio::EnumerateAudioDevices()
		{
			...
			{
				HEPH_RAISE_EXCEPTION(this, InsufficientMemoryException(HEPH_FUNC, "Insufficient memory."));
				...
				auto addSources = [this, &propertyList, &size, &deviceIDs, &cfs, &cfsLength, &result, &i, &defaultRenderID, &defaultCaptureID](AudioDeviceType deviceType) -> bool
					{
						...
						for (size_t j = 0; j < deviceSourceCount; j++)
						{
							...
							if (!CFStringGetCString(cfs, sourceName.c_str(), cfsLength + 1, kCFStringEncodingUTF8))
							{
								free(deviceSources);
								CFRelease(cfs);
								HEPH_RAISE_EXCEPTION(this, InsufficientMemoryException(HEPH_FUNC, "An error occurred while getting the device source id."));
								return AppleAudio::DEVICE_ENUMERATION_FAIL;
							}
							CFRelease(cfs);
				...
				else
				{
					...
					else
					{
						...
						if (!CFStringGetCString(cfs, device.name.c_str(), cfsLength + 1, kCFStringEncodingUTF8))
						{
							free(deviceIDs);
							CFRelease(cfs);
							HEPH_RAISE_EXCEPTION(this, InsufficientMemoryException(HEPH_FUNC, "An error occurred while getting the device name."));
							return AppleAudio::DEVICE_ENUMERATION_FAIL;
						}
						CFRelease(cfs);
		...
		void AppleAudio::ToStreamDesc(const AudioFormatInfo& format, AudioStreamBasicDescription& streamDesc) const
		{
			...
			case HEPHAUDIO_FORMAT_TAG_OPUS:
				...
				break;
			default:
				HEPH_RAISE_AND_THROW_EXCEPTION(this, InvalidArgumentException(HEPH_FUNC, "Invalid audio format."));
			}
		}
