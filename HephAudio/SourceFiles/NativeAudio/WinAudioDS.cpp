...
#define WINAUDIODS_RENDER_THREAD_EXCPT(hr, method, message) hres = hr; if(FAILED(hres)) { HEPH_RAISE_EXCEPTION(this, ExternalException(method, message, "DirectSound", WinAudioBase::GetComErrorMessage(hres))); goto RENDER_EXIT; }
#define WINAUDIODS_CAPTURE_THREAD_EXCPT(hr, method, message) hres = hr; if(FAILED(hres)) { HEPH_RAISE_EXCEPTION(this, ExternalException(method, message, "DirectSound", WinAudioBase::GetComErrorMessage(hres))); goto CAPTURE_EXIT; }
#define WINAUDIODS_ENUMERATE_DEVICE_EXCPT(hr, method, message) hres = hr; if(FAILED(hres)) { HEPH_RAISE_EXCEPTION(this, ExternalException(method, message, "DirectSound", WinAudioBase::GetComErrorMessage(hres))); return NativeAudio::DEVICE_ENUMERATION_FAIL; }
#define WINAUDIODS_ENUMERATION_CALLBACK_EXCPT(hr, winAudioDS, method, message) hres = hr; if(FAILED(hres)) { HEPH_RAISE_EXCEPTION(winAudioDS, ExternalException(method, message, "DirectSound", WinAudioBase::GetComErrorMessage(hres))); return NativeAudio::DEVICE_ENUMERATION_FAIL; }
...
namespace HephAudio
{
	namespace Native
	{
		...
		void WinAudioDS::SetMasterVolume(double volume)
		{
			if (this->isRenderInitialized)
			{
				...
				if (mmResult != MMSYSERR_NOERROR)
				{
					...
					HEPH_RAISE_EXCEPTION(this, ExternalException(HEPH_FUNC, "An error occurred while setting the master volume", "MMEAPI", errorMessage));
				}
			}
		}
		double WinAudioDS::GetMasterVolume() const
		{
			if (this->isRenderInitialized)
			{
				...
				if (mmResult != MMSYSERR_NOERROR)
				{
					...
					HEPH_RAISE_EXCEPTION(this, ExternalException(HEPH_FUNC, "An error occurred while getting the master volume", "MMEAPI", errorMessage));
					return -1.0;
				}
		...
		{
			HEPH_RAISE_EXCEPTION(this, NotSupportedException(HEPH_FUNC, "Native params not supported."));
		}
		...
		RENDER_EXIT:
			for (size_t i = 0; i < notificationCount; i++)
			{
				if (hEvents[i] != nullptr)
				{
					...
					{
						HEPH_RAISE_EXCEPTION(this, Exception(HEPH_FUNC, "An error occurred while closing the render event handles."));
					}
					hEvents[i] = nullptr;
				}
			}
		...
		CAPTURE_EXIT:
			for (size_t i = 0; i < notificationCount; i++)
			{
				if (hEvents[i] != nullptr)
				{
					...
					if (eventResult == FALSE)
					{
						HEPH_RAISE_EXCEPTION(this, Exception(HEPH_FUNC, "An error occurred while closing the capture event handles."));
					}
					hEvents[i] = nullptr;
				}
			}
