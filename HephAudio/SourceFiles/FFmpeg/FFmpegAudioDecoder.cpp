...
namespace HephAudio
{
	...
	bool FFmpegAudioDecoder::Seek(size_t frameIndex)
	{
		...
		{
			HEPH_RAISE_EXCEPTION(this, InvalidArgumentException(HEPH_FUNC, "frameIndex out of bounds."));
			return false;
		}
		...
		if (ret < 0)
		{
			HEPH_RAISE_EXCEPTION(this, ExternalException(HEPH_FUNC, "Failed to seek frame.", "FFmpeg", HEPHAUDIO_FFMPEG_GET_ERROR_MESSAGE(ret)));
			return false;
		}
		return true;
	}
	...
	AudioBuffer FFmpegAudioDecoder::Decode(size_t frameCount)
	{
		if (!this->IsFileOpen())
		{
			HEPH_RAISE_EXCEPTION(this, InvalidOperationException(HEPH_FUNC, "No open file to decode."));
				...
				while (avcodec_receive_frame(this->avCodecContext, this->avFrame) >= 0)
				{
					...
					if (currentFrameCount > 0)
					{
						...
						if (ret < 0)
						{
							...
							HEPH_RAISE_AND_THROW_EXCEPTION(this, ExternalException(HEPH_FUNC, "Failed to decode samples.", "FFmpeg", HEPHAUDIO_FFMPEG_GET_ERROR_MESSAGE(ret)));
						}
	...
	AudioBuffer FFmpegAudioDecoder::Decode(const EncodedAudioBuffer& encodedBuffer)
	{
		...
		if (pFFmpegBuffer == nullptr)
		{
			...
			{
				HEPH_RAISE_AND_THROW_EXCEPTION(this, InvalidArgumentException(HEPH_FUNC, "EncodedAudioBuffer must contain raw PCM or a-law or mu-law data"));
			}
	...
	void FFmpegAudioDecoder::OpenFile(const std::filesystem::path& filePath)
	{
		...
		{
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, ExternalException(HEPH_FUNC, "Could not open the file.", "FFmpeg", HEPHAUDIO_FFMPEG_GET_ERROR_MESSAGE(ret)));
		}
		...
		{
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, ExternalException(HEPH_FUNC, "An error occurred while finding the stream info, file might be corrupted.", "FFmpeg", HEPHAUDIO_FFMPEG_GET_ERROR_MESSAGE(ret)));
		}
		...
		{
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, NotFoundException(HEPH_FUNC, "No audio stream found."));
		}
		...
		if (avCodec == nullptr)
		{
			...
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, NotFoundException(HEPH_FUNC, "No decoder found for the " + codecName + " codec."));
		}
		...
		{
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, InsufficientMemoryException(HEPH_FUNC, "Failed to allocate codec context."));
		}
		...
		{
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, ExternalException(HEPH_FUNC, "Failed to create codec context from the codec parameters.", "FFmpeg", HEPHAUDIO_FFMPEG_GET_ERROR_MESSAGE(ret)));
		}
		...
		{
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, ExternalException(HEPH_FUNC, "Failed to open the av codec context.", "FFmpeg", HEPHAUDIO_FFMPEG_GET_ERROR_MESSAGE(ret)));
		}
		...
		{
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, InsufficientMemoryException(HEPH_FUNC, "Failed to allocate av frame."));
		}
		...
		{
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, InsufficientMemoryException(HEPH_FUNC, "Failed to allocate av packet."));
		}
		...
		{
			this->CloseFile();
			HEPH_RAISE_AND_THROW_EXCEPTION(this, ExternalException(HEPH_FUNC, "Failed to get pts of the first packet.", "FFmpeg", HEPHAUDIO_FFMPEG_GET_ERROR_MESSAGE(ret)));
		}
