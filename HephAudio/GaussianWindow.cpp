...
namespace HephAudio
{
	...
	heph_float GaussianWindow::operator[](size_t n) const noexcept
	{
		return exp(-0.5 * pow((n - this->hN) / this->shN, 2));
	}
	void GaussianWindow::SetSize(size_t newSize) noexcept
	{
		this->size = newSize;
		this->hN = 0.5 * (this->size - 1);
		this->shN = this->sigma * this->hN;
	}
	heph_float GaussianWindow::GetSigma() const noexcept
	{
		return this->sigma;
	}
	void GaussianWindow::SetSigma(heph_float sigma) noexcept
	{
		this->sigma = sigma;
		this->shN = this->sigma * this->hN;
	}
