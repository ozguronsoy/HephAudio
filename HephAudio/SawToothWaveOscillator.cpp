...
namespace HephAudio
{
	SawToothWaveOscillator::SawToothWaveOscillator(uint32_t sampleRate) : Oscillator(0.05hf, 1500.0hf, sampleRate, 0.0hf, AngleUnit::Radian) {}
	...
	heph_float SawToothWaveOscillator::Oscillate(size_t t_sample) const noexcept
	{
		...
		return this->peakAmplitude * 2.0hf * (this->frequency * t_second - round(this->frequency * t_second));
	}
}