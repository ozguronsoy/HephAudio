...
namespace HephAudio
{
		...
		: frameCount(frameCount)
	{
		if (this->frameCount > 0)
		{
			...
			{
				throw HephCommon::HephException(E_OUTOFMEMORY, "FloatBuffer::FloatBuffer", "Insufficient memory.");
			}
			this->Reset();
		}
		else
	...
	void FloatBuffer::Join(const FloatBuffer& rhs)
	{
		if (rhs.frameCount > 0)
		{
			...
			if (tempPtr == nullptr)
			{
				throw HephCommon::HephException(E_OUTOFMEMORY, "FloatBuffer::Join", "Insufficient memory.");
			}
	...
	void FloatBuffer::Insert(const FloatBuffer& rhs, size_t frameIndex)
	{
		if (rhs.frameCount > 0)
		{
			...
			if (tempPtr == nullptr)
			{
				throw HephCommon::HephException(E_OUTOFMEMORY, "FloatBuffer::Insert", "Insufficient memory.");
			}
	...
	void FloatBuffer::Cut(size_t frameIndex, size_t frameCount)
	{
		if (frameCount > 0 && frameIndex < this->frameCount)
		{
			...
			if (tempPtr == nullptr)
			{
				throw HephCommon::HephException(E_OUTOFMEMORY, "FloatBuffer::Cut", "Insufficient memory.");
			}
	...
	void FloatBuffer::Resize(size_t newFrameCount)
	{
		if (newFrameCount != this->frameCount)
		{
			...
			else
			{
				...
				if (tempPtr == nullptr)
				{
					throw HephCommon::HephException(E_OUTOFMEMORY, "FloatBuffer::Resize", "Insufficient memory.");
				}
				this->pData = tempPtr;
				this->frameCount = newFrameCount;
			}
